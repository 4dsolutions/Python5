...
Add "series" as another fact to track regarding each
Element
...
class ElementEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        if isinstance(obj, Element):  # how to encode an Element
            return [obj.protons, obj.abbrev, obj.long_name, obj.mass]
...
def load_elements():
    ...
    for symbol, data in the_dict.items():
        all_elements[symbol] = Atom(*data) # "explode" data into 4 inputs, 5 inputs
...
def print_table():
    ...
    What about series?
    """
    ordered_elements = sorted(all_elements.values(), key = lambda k: k.mass)
